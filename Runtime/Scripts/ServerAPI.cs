...
namespace AgileLiteracy.API
{
    ...
    public struct ServerRequest
    {
        ...
        {
            if (useAuthToken && AuthToken.current == null)
            {
        ...
        public static void ResponseHandler<T>(ServerResponse response, string key = null, System.Action<T> onComplete = null)
        {
            ...
            {
                Debug.LogError("Response:  " + JsonConvert.SerializeObject(response));
    ...
    public class ServerAPI : MonoBehaviour
    {
        ...
        protected IEnumerator DoAWSFileUpload(string signedUrl, ImportedFileData fileData, System.Action<float> OnProgressUpdated = null, System.Action<bool,string> OnComplete = null)
        {
            byte[] fileBytes = File.ReadAllBytes(fileData.filePath);

            using (UnityWebRequest www = UnityWebRequest.Put(signedUrl, fileBytes))
            {
                www.method = "PUT";
                ...
                www.SetRequestHeader("ContentType", fileData.type);

                UnityWebRequestAsyncOperation op = www.SendWebRequest();
                while(!op.isDone && !op.webRequest.isNetworkError && !op.webRequest.isHttpError)
                {
                    OnProgressUpdated?.Invoke(op.progress);
                    yield return null;
                }

                if(op.webRequest.isNetworkError || op.webRequest.isHttpError)
                {
                    OnComplete?.Invoke(false, op.webRequest.error);
                }
                else
                {
                    Debug.LogError($"AWS RESPONSE: {op.webRequest.downloadHandler.text}");
                    OnComplete?.Invoke(true, op.webRequest.downloadHandler.text);
                }
        ...
        protected IEnumerator DoDownloadTexture2D(string url, System.Action<Texture2D> OnComplete = null)
        {
            using (UnityWebRequest www = UnityWebRequestTexture.GetTexture(url))
            {
                ...
                www.SetRequestHeader("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT, OPTIONS, HEAD");
                www.SetRequestHeader("Access-Control-Allow-Origin", "*");
                //www.SetRequestHeader("Host", TeamFile.AMAZON_S3_HOST);
        ...
        protected IEnumerator DoSendAPIRequest(ServerRequest request, System.Action<ServerResponse> OnServerResponse = null)
        {
            ...
            using (UnityWebRequest webRequest = CreateWebRequest(url, request))
            {
                ...
                try
                {
                    string StatusColor = webRequest.responseCode == 200 ? "White" : "Red";
                    Debug.Log($"Server Response: <color={StatusColor}>{webRequest.responseCode}</color> {request.httpMethod} {request.endpoint} completed in {(Time.time - startTime).ToString("n4")} secs. Response: {webRequest.downloadHandler.text}");
                }
                catch
                {
